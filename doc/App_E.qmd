...
---
# Overview
This document is a collection of notes on the structure and behavior of key objects used
...
These names were modified from the original `cpuescore` functions to
provide more clarity. Future iterations may refactor this naming convention
to reflect that they are more generally indices (e.g., from surveys). This
may avoid confusion terminology that relates to fishery-dependent indices--i.e., CPUEs.
...
The `Slick` software, in addition to providing an interface for examining OMs and 
MPs, also provides some helper functions to visualize the results of the
MPs simulations directly. The examples in the following code chunk illustrate some
of these features.
...
The `R code` below demonstrates how to load the operating model (OM) and
compute baseline performance metrics so that the 
OM performance with MP results using the `getSlick()` function can be run.
