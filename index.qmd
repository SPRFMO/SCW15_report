...
developed under South Pacific Regional Fisheries Management Organisation (SPRFMO). The primary goal of the workshop was to ensure that the modeling framework and management procedures (Management Procedures (MPs)) are
scientifically sound, technically robust and transparent, and aligned with
management priorities as outlined by the Commission (e.g.,
@sprfmo2025jmwgmse. The workshop was successful in testing the framework, 
...
the workshop contributed to small changes made in the conditioning 
of the Operating Models (OMs) to improve the robustness of the results. 
...
2.  **MSE and OM fine-tuning**\
    ...
    future behaviour. Participants discussed and implemented changes to the OM structure,
    including adjustments to transitions to recruitment dynamics, and 
...
process into three main stages noting that in stage 1, a 
key activity is to "tune" MPs towards a common goal (@fig-mp_flow). Here
that goal (based on the questionaire distributed to members durin COMM13) was to 
achieve a 60% probability of remaining in the "green zone" (i.e., above the $B_{MSY}$
reference point) in the final years of the projection period. 
...
exercise were available and implemented. However, the group encountered 
difficulties ascertaining how the TACs related to the empirical MPs that
were being tested. 
...
The following sections first review the OM and some results from applying existing
and refined MPs.  We follow this with 
the discussion on how best to incorporate
...
We conclude with a set of recommendations for the SC to consider.
...
![Distribution of reference points from the operating model accepted by
the workshop.](images/ref_pts.png){#fig-ref_pts}
...
fleets, we analyzed catch data from 2004 to 2024. These data were
smoothed using a 5-year moving average to reduce annual variability.
...
(2006–2010) to a low of 6% (2018–2024), representing a 20 percentage 
...
evaluate_mp(). The performance2() function extends standard summary outputs by
computing a richer set of indicators—such as mean relative biomass and fishing
...
Rule (HCR) settings, and it incorporates a customizable objective function that
accounts for discounted catch, stability (via IACV), conservation thresholds
(e.g., probability of being “green”), and duration outside target reference
points. This setup allows for filtering out implausible simulations, facilitates
...
values, the control object from [Section @sec-def] is tested across a
set of TAC multipliers. The target range brackets the 2025 CMM level.
...
is a common requirement in many fisheries management systems. These
...
The performance of shortcut management procedures (MPs) over the tuning
...
asymmetric −25%/+15% constraint (d2515). While mean catch (mean(C)) is
similar across all three MPs, the application of TAC constraints notably
reduces interannual variability (IAC(C)) compared to the unconstrained
...
(P(TAClimit)) for d15 and d2515. Spawning biomass (SB) and fishing
mortality (F) remain broadly similar across scenarios, suggesting that
...
expected increases in catch, but also in fishing mortality (F) and
interannual catch variability (IAC(C)), with slight declines in spawning
biomass (SB). All options show negligible probability of hitting the TAC
floor (P(TAClimit)). In contrast, comparing MPs with the same TAC target
...
panel), differences in fishing mortality (F), interannual catch
variability (IAC(C)), and spawning biomass (SB) across catch targets
...
#### Shortcut sensitivity analysis {#sec-shortcut}

The shortcut method is designed to simplify the estimation of a stock assessment process
and the results shown in the previous section illustrate how this can be reflected based
on performance metrics. Here we test the sensitivity of the shortcut method if in fact
the "noisiness" is greater and if there is a bias in stock size estimates. 
...
(@fig-sc_alts). Similarly, except for the inter-annual catch variability (IAC(C)), 
the performance indicators were unaffected by the
banking and borrow (as part of the implementation error specification; @fig-bank_borrow). 
"Banking" TAC tended to lower the inter-annual variability in catch whereas "borrowing"
increased it. 
...
configured and evaluated in the jmMSE framework. The use of buffered
